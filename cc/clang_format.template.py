...
Template variables (baked in by clang_format.bzl):
    ...
    {config} — path to .clang-format config
"""
...
def find_tool(label="clang-format"):
...
def parse_args(argv):
    ...
    return check_mode, explicit_files, tool_flags
...
def main():
    ...
    check_mode, explicit_files, tool_flags = parse_args(sys.argv)
    if explicit_files:
    ...
    if check_mode:
        cmd = [tool, "--dry-run", "--Werror"] + tool_flags + sources
        result = subprocess.run(cmd, cwd=workspace_root)
        if result.returncode != 0:
            print(f"\033[0;31m{len(sources)} file(s) checked, formatting issues found.\033[0m", file=sys.stderr)
            sys.exit(1)
        print(f"\033[0;32m{len(sources)} file(s) checked, all formatted correctly.\033[0m")
    else:
        config_args = []
        has_style = any(f.startswith("--style") for f in tool_flags)
        if not has_style:
            config_path = os.path.join(workspace_root, CONFIG)
            if os.path.exists(config_path):
                config_args = [f"--style=file:{config_path}"]
        cmd = [tool, "-i"] + config_args + tool_flags + sources
        result = subprocess.run(cmd, cwd=workspace_root)
        if result.returncode != 0:
            print(f"\033[0;31mFormatting failed with exit code {result.returncode}.\033[0m", file=sys.stderr)
