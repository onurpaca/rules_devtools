...
Template variables (baked in by black.bzl):
    ...
    {config} — path to pyproject.toml config
"""
...
def main():
    ...
    if not has_config:
        ...
        if os.path.exists(config_path):
            config_args = [f"--config={config_path}"]

    if check_mode:
        cmd = [tool, "--check"] + config_args + tool_flags + sources
