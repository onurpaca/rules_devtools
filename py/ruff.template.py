"""Format and lint Python source files using ruff.

Tool resolution: hermetic first (if configured), PATH fallback (with warn).

Template variables (baked in by ruff.bzl):
...
def parse_args(argv):
    ...
    return check_mode, fix_mode, explicit_files, tool_flags
...
def main():
    ...
    check_mode, fix_mode, explicit_files, tool_flags = parse_args(sys.argv)
    if explicit_files:
        sources = expand_paths(explicit_files, workspace_root)
        if not sources:
            print("\033[0;33mNo source files found under:\033[0m", explicit_files)
            return

    if check_mode:
        cmd = [tool, "check"] + tool_flags + sources
        result = subprocess.run(cmd, cwd=workspace_root)
        if result.returncode != 0:
            print(f"\033[0;31m{len(sources)} file(s) checked, linting issues found.\033[0m", file=sys.stderr)
            sys.exit(1)
        print(f"\033[0;32m{len(sources)} file(s) checked, all clean.\033[0m")
    elif fix_mode:
        cmd = [tool, "check", "--fix"] + tool_flags + sources
        result = subprocess.run(cmd, cwd=workspace_root)
        print(f"\033[0;32m{len(sources)} file(s) processed.\033[0m")
    else:
        cmd = [tool, "format"] + tool_flags + sources
        result = subprocess.run(cmd, cwd=workspace_root)
        if result.returncode != 0:
            print(f"\033[0;31mRuff formatting failed with exit code {result.returncode}.\033[0m", file=sys.stderr)
            sys.exit(result.returncode)
        print(f"\033[0;32m{len(sources)} file(s) formatted.\033[0m")
